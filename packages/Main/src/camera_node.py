...
BASE_SPEED = 0.30          # Reduced base speed for better control
...
MAX_STEER = 0.4            # Reduced maximum steering adjustment
SMOOTHING_STRAIGHT = 5     # Increased smoothing for straight roads
SMOOTHING_CURVE = 3        # Moderate smoothing for curves
TURN_SLOWDOWN_FACTOR = 0.5 # Factor to slow down during turns
...
class CameraReaderNode(DTROS):
    def __init__(self, node_name):
        ...
        # Initialize parameters and stash originals
        self.base_speed       = BASE_SPEED
        self.curve_speed      = CURVE_SPEED
        self._base_speed_orig  = self.base_speed
        self._curve_speed_orig = self.curve_speed
        ...
        self._sign_subscriber = rospy.Subscriber("sign", String, self.sign_detected)

        self.shutting_down = False
        rospy.on_shutdown(self.shutdown_hook)

    def sign_detected(self, msg):
        ...
        if(msg.data == "Slow Down"):
            ...
            rospy.sleep(10)
            self._restore_speeds(rospy.Time.now())
            # self._sign_subscriber = rospy.Subscriber(
            #     "sign", String, self.sign_detected, queue_size=1
            # )

        if(msg.data == "Stop"):
            rospy.loginfo(f"[{self.node_name}] sign detected: {msg.data}, stopping for 5")
            # scale speeds down (e.g. to 20%):
            self.base_speed  = self._base_speed_orig  * 0
            self.curve_speed = self._curve_speed_orig * 0
    ...
    def _restore_speeds(self, event):
        rospy.loginfo(f"[{self.node_name}] restoring normal speeds")
        self.base_speed  = self._base_speed_orig
        self.curve_speed = self._curve_speed_orig
    ...
    def callback(self, msg):
        ...
        roi_top = int(h*0.60)  # ROI starts at 60% down from top
        ...
        lb_yellow = np.array([10, 50, 160]) # WAS 85
        ...
        if "yellow_only" in following_mode and yellow_centroid is not None:
            ...
            if yellow_curvature < -20:  # Significant left curve
                ...
                curve_adjustment = -1 * (abs(yellow_curvature) / 50) # <--------------------- CHANGED HERE E.K. was (-0.1)
